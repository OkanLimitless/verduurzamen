      ...
      <div className="mt-3 grid grid-cols-2 gap-3 text-sm">
        <div className="rounded-2xl border border-neutral-200 bg-white shadow-lg p-4">
          ...
          <ul className="mt-2 list-none">
            {pros.map((p) => (
              <li key={p} className="flex items-center gap-2 text-[var(--muted)]"><Check className="size-4 text-emerald-600" aria-hidden /> {p}</li>
            ))}
          </ul>
        </div>
        <div className="rounded-2xl border border-neutral-200 bg-white shadow-lg p-4">
          ...
          <ul className="mt-2 list-none">
            {cons.map((c) => (
              <li key={c} className="flex items-center gap-2 text-[var(--muted)]"><X className="size-4 text-red-600" aria-hidden /> {c}</li>
            ))}
          </ul>
        </div>
      </div>
